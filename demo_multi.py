...
def excel_predict(config,stopwords):
    ...
    while True:
        ...
        if os.path.isfile(file_path) and (file_path.endswith('.xlsx') or file_path.endswith('.xls')):
            test_path,sheet_names = Excel_preddata().multi_chuli(stopwords,file_path) #multi_chuli多进程 chuli(stopwords,file_path)
...
def excel_test(config,stopwords):
    ...
    while True:
        ...
        if os.path.isfile(file_path) and (file_path.endswith('.xlsx') or file_path.endswith('.xls')):
            test_path = Excel_testdata().multi_chuli(stopwords,file_path) #multi_chuli多进程 chuli(stopwords,file_path)
        else:
            print('路径错误或文件格式错误，请重新输入')
            continue
        if test_path:
